...
HWND CreateWin32Window(HINSTANCE hInstance, int width, int height, int x, int y, bool fullscreen)
{
	...
	// initialise window
	HWND hWnd = CreateWindowW(L"OPENGLWINDOW", L"JPHP App Game Kit", dwStyle, x, y,
...
public:
	...
	void Init(JNIEnv* jenv, jobject jobj , int deviceWidth = 800 ,int deviceHeight = 600, bool fullscreen = false) {
		HINSTANCE hInstence = (HINSTANCE)GetModuleHandle(NULL);

		// Создаем окно
		int dwCenterWindowX = (GetSystemMetrics(SM_CXSCREEN) - (int)deviceWidth) / 2;
		int dwCenterWindowY = (GetSystemMetrics(SM_CYSCREEN) - (int)deviceHeight) / 2;
		HWND hWnd = CreateWin32Window(hInstence, deviceWidth, deviceHeight, dwCenterWindowX, dwCenterWindowY, fullscreen);

		agk::SetCompanyName("Jphp App Game Kit");

		//получение дескриптора класса 
		jclass clazz = jenv->GetObjectClass(jobj);

		//получение идентификатора метода 
		jmethodID Begin = jenv->GetMethodID(clazz, "Begin", "()V");
		jmethodID Loop = jenv->GetMethodID(clazz, "Loop", "()V");
		jmethodID End = jenv->GetMethodID(clazz, "End", "()V");
		...
		try
		{
			...
			jenv->CallVoidMethod(jobj, Begin);
		}
		catch (...)
		{
